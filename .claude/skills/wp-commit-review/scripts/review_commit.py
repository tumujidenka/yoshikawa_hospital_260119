...
- HTML: セマンティックなタグ、意味のある画像へのalt属性の付与などアクセシビリティ、不要なインラインスタイル
- CSS/SCSS: 命名規則(BEM等)の統一、レスポンシブ対応、未使用・重複スタイル
- SEO: 変更差分にtitle/meta description/OGP/構造化データ(JSON-LD)を出力するコードが含まれる場合、内容の整合性(重複・空文字・ハードコードされたURL等)
